...
class CmisManagerTask(threading.Thread):
    ...
    def handle_cmis_dp_init_state(self, lport):
        """
        Handle the CMIS_STATE_DP_INIT state for a logical port.
    ...
    def handle_cmis_dp_txon_state(self, lport):
        """
        Handle the CMIS_STATE_DP_TXON state for a logical port.
    ...
    def process_cmis_state_machine(self, lport):
        ...
        try:
            ...
            if state == CMIS_STATE_DP_PRE_INIT_CHECK:
                ...
                if not self.handle_cmis_dp_init_state(lport):
                    return
            elif state == CMIS_STATE_DP_TXON:
                if not self.handle_cmis_dp_txon_state(lport):
                    return
